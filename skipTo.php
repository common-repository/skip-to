...
*/
	...
	$skipTo_url = '/wp-content/plugins/skip-to/js/SkipTo.min.js';
	...
	function skipTo_options() {
		...
		if(!empty($_POST)){
			...
			if(!empty($post_headings) or !empty($post_roles)) {
				...
				if(!empty($_POST['idsandclass'])) $arrw['settings']['skipTo']['ids'] = $_POST['idsandclass'];
				if(!empty($_POST['customclassc'])) $arrw['settings']['skipTo']['customClass'] = $_POST['customclass'];
				if(!empty($_POST['attachelement'])) $arrw['settings']['skipTo']['attachElement'] = $_POST['attachelement'];
